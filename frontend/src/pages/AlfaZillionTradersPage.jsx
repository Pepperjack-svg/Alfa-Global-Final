...
const AlfaZillionTradersPage = ({ theme = 'gold' }) => {
  ...
  const services = [
    { title: 'Fintech Trading Platforms', desc: 'Access next gen fintech platforms that simplify trading and maximize returns.' },
    { title: 'AI Powered Strategies', desc: 'Leverage proprietary AI driven models for smarter, faster, and more informed trading decisions.' },
    { title: 'Alpha Engineering', desc: 'Our exclusive alpha engineering strategies are designed to generate consistent, risk adjusted returns for select partners.' },
    { title: 'Global Market Access', desc: 'Gain seamless access to international markets, cross border investments, and global trading opportunities.' }
  ];
        ...
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-12">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
              <img src="https://websitesbytechpioneers.com/alfaglobalnew/image/alfa-zillion-traders.png" alt="Alfa Zillion Traders" className="rounded-2xl shadow-xl w-full" />
            </motion.div>
            <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
      ...
      <section className={`py-16 ${t.accentBgLight}`}>
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-12 text-center">
          <Link to="/contact" className={`inline-flex items-center gap-2 px-8 py-4 ${t.accentBg} text-white font-semibold rounded-lg hover:opacity-90 transition-all`}>
            Start Trading With Us <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M17 8l4 4m0 0l-4 4m4-4H3"/></svg>
          </Link>
        </div>
      </section>
    </div>
